...
## Current state — Rev 4
Working: **BOM import (CSV/TSV/Excel-paste or file) with auto column-mapping**, **nominal
footprint resolver** keyed by circuit type / description / PN, **auto-arrange** (DIN devices packed
along rails with wrap; large panel-mount gear packed into the open bays between rails; duct
keep-outs avoided), load embedded sample, 2D drag with grid + DIN-rail snap, keyboard nudge,
duct/overlap/envelope keep-out guard (caution-yellow flag + clickable status list), 3D orbit/zoom
view, inspector edit of all device fields, Back/Door face toggle, Import/Export JSON, add/rotate/
delete device, and **Export Drawings (HTML)** = one self-contained file with a back-panel 2D
elevation (red mm dims to the lower-left datum), a door 2D elevation, an interactive 3D view, and a
device/nameplate schedule table.
Stubbed / known: device dimensions are nominal placeholder boxes, not real footprints
(dimsSource:"nominal"); PNs are representative; header uses a green wordmark, not the real
shield asset (not yet supplied — logo slot is reserved, never ship a placeholder logo).

## Data contract (the JSON model — single source of truth)
Top level: schemaVersion, meta, rails[], ducts[], devices[].
- meta: so, panelId, rev, units:"mm", datum:"lower-left", enclosure{mfg,pn,typeRating,width,height,depth}, subpanel{width,height}, door{present,width,height}.
- rails[]: id, face, type, orientation, x, y, length — these are the snap targets.
- ducts[]: id, face, x, y, w, h, depth — these are keep-out zones.
- devices[]: id, tag, pn, description, circuitType, face("back"|"door"), mount("din"|"panel"), railId, geometry{x,y,w,h,depth,rotation}, dimsSource("nominal"|"catalog"), clearanceMm, nameplate.
...
- face: "back" = subpanel; "door" = door inner face. Each face has its own envelope.
...
1. Replace nominal dims with real footprint dims from the catalog (Access .mdb lookup by
   PN); flip dimsSource to "catalog" so the keep-out guard becomes trustworthy.
...
- EC-PanelLayout_Editor_Rev4.html — current app (start here).
